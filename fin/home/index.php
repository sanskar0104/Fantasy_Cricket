<?php
 session_start();
?>
<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>FCG | Home</title>
    <script defer src="https://kit.fontawesome.com/b8be333c62.js" crossorigin="anonymous"></script>
    <link rel="icon" href="../logo-min.PNG">
    <link rel="stylesheet" href="../common.css">
    <link rel="stylesheet" href="main.css">
</head>
<body>
<script>
        var grsl = document.getElementsByClassName("grsl");
        var current = 0;
        document.addEventListener('keydown', function(e) {
            switch (e.keyCode) {
                case 37:
                    goup();
                    break;
                case 39:
                    godown();
                    break;
                case 78:
                    document.getElementById("navbar").classList.toggle("disnon");
                    break;
            }
        });

        function godown() {
            if (current < (grsl.length - 1)) {
                current++;
                grsl[current].scrollIntoView({
                    behavior: 'smooth'
                });
            }
        }

        function goup() {
            if (current > 0) {
                current--;
                grsl[current].scrollIntoView({
                    behavior: 'smooth'
                });
            }
        }
    </script>
    <div id="navbar">
        <div class="flex-row nav-bar">
            <div class="right">
                <div class="img curpoi" onclick="window.location.assign('../home')"><img class="curpoi" src="../logo.PNG" alt=""></div>
            </div>
            <div class="left">
                <div class="disp-lap-nav">
                    <div class="flex-row width-fit">
                        <div  onmouseover="noneall_nav()"onclick="window.location.assign('../home')" class="nav-item nav-acti  curpoi">
                            <div class="flex-row">
                                <p class="">Home</p>
                            </div>
                        </div>
                        <div  onmouseover="noneall_nav()"onclick="window.location.assign('../how')" class="nav-item curpoi">
                            <div class="flex-row">
                                <p class="">How to</p>
                            </div>
                        </div>
                        <div onmouseover="noneall_nav()" onclick="window.location.assign('../player')" class="nav-item curpoi">
                            <div class="flex-row">
                                <p class="">Players profile</p>
                            </div>
                        </div>
            
                        <div onmouseover="visible_nav(0)" class="nav-item  curpoi">
                            <div class="flex-row"><p id="varUN" class="">Login
                                </p>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>

        <div  class="mega-menu disp-lap-nav">

            <div  class=" nav-drop-down">
                <div id="drpdw" class="disnon">
                <div class="sam-nav">
                    <div class="details comp">
                        <div >
                            <p id="varFN" class="varF"></p>
                        </div>
                       <div>
                        <p id="varEM" class="varE"></p>
                        </div>

                    </div>
                    <div class="logout comp"> 
                    <div ><p id="lout"><a  href="../logout.php">logout</a></p>
                    </div>
                    </div>
                </div></div>
            </div>
        </div>
    </div>


    <div class="body-div" onmouseover="noneall_nav()">
        <div id="contentbx"class="contentbx flex-row">
            <form  autocomplete="off"  action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" method="POST">
                <div class="contan">
                    <h2 class="black1 it-log">Login</h2>
                    <h2 class="black1 it-reg disnon">Registration</h2>
                    <br>
                    <label class="container">Login
                    <input type="radio" value="login" checked="checked" name="type" onchange="switchLaR()">
                    <span class="checkmark pad-sid"></span>
                    </label>
                    <label class="container">Registration
                    <input type="radio" value="registration" name="type" onchange="switchLaR()">
                    <span class="checkmark"></span>
                    </label>
                    <br>
                    <br>
                    <div class="inputbx ">
                        <input class="form it-reg disnon" id="username" type="text" name="username" placeholder="Enter User name" spellcheck="false" autocomplete="off">
                        <input class="form it-reg disnon" id="fullname" type="text" name="fullname" placeholder="Enter Full name" spellcheck="false" autocomplete="off">
                        <input class="form" id="E-Mail" type="text" name="E-Mail" placeholder="Enter E-Mail" spellcheck="false" autocomplete="off" required="">
                        <input class="form" id="password" type="password" name="pword" placeholder="Enter Password" spellcheck="false" autocomplete="off" required="">

                    </div>
                </div>
                <div><button class="buto-1 curpoi" style="border:none;"  type="submit" name="submit"><h2 class="tecenter white1 curpoi">Submit</h2> </button>
                </div>
            </form>
        </div>


        <div class="man-slid grsl">
            <div class="cir-1"></div>
            <div class="cir-2"></div>
            <div class="cir-3"></div>
            <div class="logo var"><img src="logo.PNG"></div>
            <div class="flex-row var text1 white1 tecenter">
                <h1>Fantasy Cricket Game</h1>
            </div>
            <div class="flex-row var text2 white1 tecenter">
                <h1>We have developed an python project</h1>
            </div>
            <div class="flex-row var text3 white1">
                <h1>FCG is a game to test your bond of analytical skills and luck. It prices you for player you choose and pay you the returns based on their performance.</h1>
            </div>
            <div class="flex-row img1 var">
                <img src="img/head.jpg" alt="">
            </div>

            <div class="flex-row var text4 white1 tecenter">
                <h1>Roles</h1>
            </div>
            <div class="flex-col var count1 cont white1 tecenter">
                <h1 id="dg-ct-1">Rohit</h1>
                <h2>UI & Testing</h2>
            </div>
            <div class="flex-col var count2 cont white1 tecenter">
                <h1>Nirvan</h1>
                <h2>Database & Web</h2>
            </div>
            <div class="flex-col var count3 cont white1 tecenter">
                <h1>Sanskar</h1>
                <h2>Python</h2>
            </div>
            <div class="flex-col var count4 cont white1 tecenter">
                <h1>Pranay</h1>
                <h2>Web & Database</h2>
            </div>
        </div>

        <div class="tech grsl">
            <div>
                <div class="flex-row">
                    <div class="inline">
                        <h1 class="tecenter">TECHNOLOGIES</h1>
                        <div class="h-lin"></div>
                    </div>
                </div>
                <br><br>
                <div class="comp">
                    <div class="card card-hover">
                        <img class="limg" src="img/1.png" alt="python" /></div>
                    <div class="card card-hover">
                        <img class="limg" src="img/2.png" alt="MySQL" /> </div>
                    <div class="card card-hover">
                        <img class="limg" src="img/3.png" alt="PHP" /> </div>
                    <div class="card card-hover">
                        <img class="pimg" src="img/4.png" alt="HTML" /> </div>
                    <div class="card card-hover">
                        <img class="pimg" src="img/5.png" alt="css" /> </div>
                    <div class="card card-hover">
                        <img class="pimg" src="img/6.png" alt="js" /> </div>
                </div>
            </div>
        </div>
        <div class="tech grsl">
            <div>
                <div class="flex-row">
                    <div class="inline">
                        <h1 class="tecenter">LIBRARIES</h1>
                        <div class="h-lin"></div>
                    </div>
                </div>
                <br><br>
                <div class="comp">
                    <div class="card card-hover">
                        <img class="pimg" src="img/7.png" alt="PYQT5" /></div>
                    <div class="card card-hover">
                        <img class="limg" src="img/8.png" alt="Numpy" /> </div>
                    <div class="card card-hover">
                        <img class="limg" src="img/9.png" alt="SQLAlchemy" /> </div>
                    <div class="card card-hover">
                        <img class="limg" src="img/10.png" alt="BeautifulSoup-html" /> </div>
                    <div class="card card-hover">
                        <img class="limg" src="img/11.png" alt="pillow-images" /> </div>

                </div>
            </div>
        </div>
        <div class="tech grsl">
            <div>
                <div class="flex-row">
                    <div class="inline">
                        <h1 class="tecenter">RESOURCES</h1>
                        <div class="h-lin"></div>
                    </div>
                </div>
                <br><br>
                <div class="comp">
                    <div class="card card-hover">
                        <img class="limg" src="img/12.png" alt="YOUTUBE" /></div>
                    <div class="card card-hover">
                        <img class="limg" src="img/13.png" alt="STACKOVERFLOW" /> </div>
                    <div class="card card-hover">
                        <img class="limg" src="img/14.png" alt="GEEKSFORGEEKS" /> </div>
                    <div class="card card-hover">
                        <img class="pimg" src="img/15.png" alt="W3SCHOOLS" /> </div>
                    <div class="card card-hover">
                        <img class="pimg" src="img/16.png" alt="" /> </div>

                </div>
            </div>
        </div>


        <section class="team-div ">
            <div class="team-div-title">
                <div class="div-letters">
                    <div class="tecenter new-ttl" style="width: 100%;">
                        <h2 class="cblac333">Team</h2>
                    </div>
                    <div class="grp-links">
                        <div class="links curpoi flex-row">
                            <a href="#roh">
                                <p class="curpoi"><i class="fas fa-user-graduate"></i>&nbsp;Rohit&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;(20u02004)</p>
                            </a>
                        </div>
                        <div class="links curpoi flex-row">
                            <a href="#nir">
                                <p class="curpoi"><i class="fas fa-user-graduate"></i>&nbsp;Nirvan&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;(20u02009)</p>
                            </a>
                        </div>
                        <div class="links curpoi flex-row">
                            <a href="#san">
                                <p class="curpoi"><i class="fas fa-user-graduate"></i>&nbsp;Sanskar&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;(20u02010)</p>
                            </a>
                        </div>
                        <div class="links curpoi flex-row">
                            <a href="#pra">
                                <p class="curpoi"><i class="fas fa-user-graduate"></i>&nbsp;Pranay&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;(20u02012)</p>
                            </a>
                        </div>
                    </div>
                </div>
            </div>

            <div class="team-div-pages">
                <div class="div-team">
                    <div id="roh" class="mem-card grsl">
                        <div class="img"><img src="img/roh.jpeg" alt=""></div>
                        <div>
                            <h1 class="name teupper">Rohith</h1>
                            <h3 class="roll">20u02004</h3>
                            <h3 class="role">Management of UI, code &<br>testing process</h3>
                        </div>
                        <div class="team-social">
                            <div class="flex-row ">
                                <a href="https://www.instagram.com/rohitt_o01_/" target="_blank"><span class=" social-sym hwhite "><i class="fab fa-instagram " aria-hidden="true"></i></span></a>
                                <a href="https://www.linkedin.com/in/koppula-rohithkumar-3b3a1520b" target="_blank"><span class="social-sym hwhite"><i class="fab fa-linkedin-in " aria-hidden="true"></i></span></a>
                            </div>
                        </div>

                    </div>
                    <div id="nir" class="mem-card grsl">
                        <div class="img"><img src="img/nir.jpeg" alt=""></div>
                        <div>
                            <h1 class="name teupper">Nirvan</h1>
                            <h3 class="roll">20U02009</h3>
                            <h3 class="role">Database & Web development lead</h3>
                        </div>
                        <div class="team-social">
                            <div class="flex-row ">
                                <a href="https://www.instagram.com/its_me_versatile.nirvan" target="_blank"><span class=" social-sym hwhite "><i class="fab fa-instagram " aria-hidden="true"></i></span></a>
                                <a href="https://www.linkedin.com/in/bnirvanreddy" target="_blank"><span class="social-sym hwhite"><i class="fab fa-linkedin-in " aria-hidden="true"></i></span></a>
                                <a href="https://twitter.com/nirvan_reddy" target="_blank"><span class=" social-sym hwhite "><i class="fab fa-twitter " aria-hidden="true"></i></span></a>
                            </div>
                        </div>

                    </div>
                    <div id="san" class="mem-card grsl">
                        <div class="img"><img src="img/san.jpeg" alt=""></div>
                        <div>
                            <h1 class="name teupper">Sanskar</h1>
                            <h3 class="roll">20U02010</h3>
                            <h3 class="role">Python lead</h3>
                        </div>
                        <div class="team-social">
                            <div class="flex-row ">
                                <a href="https://www.linkedin.com/in/sanskar-asati-184290202" target="_blank"><span class=" social-sym hwhite "><i class="fab fa-instagram " aria-hidden="true"></i></span></a>
                                <a href="https://www.linkedin.com/in/sanskar-asati-184290202/" target="_blank"><span class="social-sym hwhite"><i class="fab fa-linkedin-in " aria-hidden="true"></i></span></a>
                            </div>
                        </div>

                    </div>
                    <div id="pra" class="mem-card grsl">
                        <div class="img"><img src="img/pra.JPG" alt=""></div>
                        <div>
                            <h1 class="name teupper">Pranay</h1>
                            <h3 class="roll">20U02012</h3>
                            <h3 class="role">Web development & Database lead</h3>
                        </div>
                        <div class="team-social">
                            <div class="flex-row ">
                                <a href="http://www.linkedin.com/in/rpranaykumarreddy" target="_blank"><span class="social-sym hwhite"><i class="fab fa-linkedin-in " aria-hidden="true"></i></span></a>
                                <a href="https://twitter.com/rpkr_inc" target="_blank"><span class=" social-sym hwhite "><i class="fab fa-twitter " aria-hidden="true"></i></span></a>
                            </div>
                        </div>

                    </div>
                </div>
            </div>
        </section>

    
        <div class="foot-img grsl flex-col">
            <div class="bg">
                <div class="foot-grp">
                    <div class="foot-left">
                        <div><h1 class="thank">THANK YOU</h1>
                    <h2 class="tecenter">None of the codes are copied. <br> But some inspirations led us to this project.</h2></div>
                    </div>        
                    <div class="foot-right">
                        <div class="foot-title">
                            <div class="inline">
                                <h1 class="tecenter">Team Members</h1>
                                <div class="h-lin"></div>
                            </div>
                        </div>
                               <div class="teleft"> <h2 class="curpoi">Rohit</h2>
                                <h2 class="curpoi">Nirvan</h2>
                                <h2 class="curpoi">Sanskar</h2>
                                <h2 class="curpoi">Pranay</h2></div>
                        </div>
                    </div>
                        
                    </div>
                </div>
            </div>
        </div>



        <button onclick="topFunction()" id="myBtn" title="Go to top"><i class="fas fa-arrow-circle-up"></i></button>
        <script>
            function switchLaR() {
                getlog = document.getElementsByClassName("it-log");
                getreg = document.getElementsByClassName("it-reg");
                sel = document.querySelector('input[name="type"]:checked').value;
                console.log(sel);
                if (sel == "login") {
                    for (var i = 0; i < getlog.length; i++) {
                        getlog[i].classList.remove("disnon");
                    }
                    for (var i = 0; i < getreg.length; i++) {
                        getreg[i].classList.add("disnon");
                    }
                }
                if (sel == "registration") {
                    for (var i = 0; i < getlog.length; i++) {
                        getlog[i].classList.add("disnon");
                    }
                    for (var i = 0; i < getreg.length; i++) {
                        getreg[i].classList.remove("disnon");
                    }
                }

            }


            var prevScrollpos = window.pageYOffset;
            window.onscroll = function() {
                scrollFunction();
                noneall_nav();
                var currentScrollPos = window.pageYOffset;
                if (document.body.scrollTop > 65 || document.documentElement.scrollTop > 65) {
                    if (prevScrollpos > currentScrollPos) {
                        document.getElementById("navbar").style.top = "0px";
                    } else {
                        document.getElementById("navbar").style.top = "-65px";
                    }
                    prevScrollpos = currentScrollPos;
                } else {
                    document.getElementById("navbar").style.top = "0px";
                }
            }
            var mybutton = document.getElementById("myBtn");


            function scrollFunction() {
                if (document.body.scrollTop > 65 || document.documentElement.scrollTop > 65) {
                    mybutton.style.display = "block";
                } else {
                    mybutton.style.display = "none";
                }
            }

            function topFunction() {
                document.body.scrollTop = 0;
                document.documentElement.scrollTop = 0;
            }

            
            function noneall_nav() {
                var cla = document.getElementsByClassName("nav-drop-down");
                for (var i = 0; i < cla.length; i++) {
                    cla[i].style.display = "none";
                }
            }

            function visible_nav() {
                var cla = document.getElementsByClassName("nav-drop-down");
                cla[0].style.display = "block";
            }

        </script>
        
<?php 
    echo"<script>console.log(\"open php\")</script>";
    $server="localhost";
    $username="root";
    $password="";
    $DB="reeg";
    $con=mysqli_connect($server,$username,$password,$DB);
    if(!$con)
    {die("connection to this database failed due to". mysqli_connect_error());}
    else{echo"<script>console.log(\"connection estab\")</script>";
        if(isset($_SESSION['userName'])){
            echo "<script>document.getElementById(\"contentbx\").classList.add(\"disnon\");</script>";
            $us=$_SESSION['userName'];
            $fn=$_SESSION['fullName'];
            $em=$_SESSION['eMail'];
                        echo "<script>document.getElementById(\"varUN\").innerHTML ='$us';</script>";
                        echo "<script>document.getElementById(\"varFN\").innerHTML ='$fn';</script>";
                        echo "<script>document.getElementById(\"varEM\").innerHTML ='$em';</script>";
                        echo "<script>document.getElementById(\"drpdw\").classList.remove(\"disnon\");</script>";
                        
                    }  else{
        if (isset($_POST['submit'])) {
            if($_POST['type']=="login"){
                echo"<script>console.log(\"submit open\")</script>";
                $email=$_POST['E-Mail'];
                $email=strtolower("$email");
                $pword=$_POST['pword'];
                $email_search = "SELECT * FROM `regi` where eMail='$email' ";
                $query = mysqli_query($con,$email_search);
                echo"<script>console.log(\"query done\")</script>";
                if(mysqli_num_rows($query)>0){
                    $email_pass = mysqli_fetch_assoc($query);
                    $db_pass = $email_pass['passWord'];
                    $_SESSION['userName']=$email_pass['userName'];
                    $_SESSION['fullName']=$email_pass['fullName'];
                    $_SESSION['eMail']=$email_pass['eMail'];
                    if($pword==$db_pass){
                        echo"<script>console.log(\"password match\")</script>";
                        echo"<script>document.getElementById(\"contentbx\").classList.add(\"disnon\");</script>";
                        $us=$_SESSION['userName'];
                        $fn=$_SESSION['fullName'];
                        $em=$_SESSION['eMail'];
                        echo "<script>document.getElementById(\"varUN\").innerHTML ='$us';</script>";
                        echo "<script>document.getElementById(\"varFN\").innerHTML ='$fn';</script>";
                        echo "<script>document.getElementById(\"varEM\").innerHTML ='$em';</script>";
                        echo "<script>document.getElementById(\"drpdw\").classList.remove(\"disnon\");</script>";
                        
                    }
                    else{
                        echo "<script>alert(\"Incorrect credentials\")</script>";
                    }
                }
                else{
                    echo "<script>alert(\"E-mail does not exist\")</script>";
                }
            }
            else if($_POST['type']=="registration"  && (!(isset($_SESSION['userName'])))){
                echo"<script>console.log(\"regis open\")</script>";
                $id=$_POST['username'];
                $id=strtoupper("$id");
                $name=$_POST['fullname'];
                $email=$_POST['E-Mail'];
                $email=strtolower("$email");
                $pword=$_POST['pword'];
               
                $email_search = "SELECT * FROM `regi` where eMail='$email' or userName='$id' ";
                $query = mysqli_query($con,$email_search);
                $email_pass = mysqli_fetch_assoc($query);
                if ($email_pass) { 
                    if ($email_pass['eMail'] == $email) {
                        echo "<script>alert(\"E-mail already exist\")</script>";
                    }
                    if ($email_pass['userName'] == $id) {
                        echo "<script>alert(\"Username already exist\")</script>";
                    }
                } else{

                 
                $sql = "INSERT INTO regi (`userName`, `fullName`, `eMail`, `passWord`) VALUES
                ('$id', '$name', '$email', '$pword');";
                mysqli_query($con, $sql);


                $signup= "SELECT * FROM `regi` where eMail='$email' and passWord ='$pword' ";
                $result = mysqli_query($con,$signup);
                if( mysqli_num_rows($result)>0){
                    $signup_pass=mysqli_fetch_assoc($result);
                    $db_pass = $signup_pass['passWord'];
                    $_SESSION['userName']=$signup_pass['userName'];
                    $_SESSION['fullName']=$signup_pass['fullName'];
                    $_SESSION['eMail']=$signup_pass['eMail'];
                    if($pword==$db_pass){
                        echo"<script>console.log(\"password match\")</script>";
                        echo"<script>document.getElementById(\"contentbx\").classList.add(\"disnon\");</script>";
                        $us=$_SESSION['userName'];
                        $fn=$_SESSION['fullName'];
                        $em=$_SESSION['eMail'];
                        echo "<script>document.getElementById(\"varUN\").innerHTML ='$us';</script>";
                        echo "<script>document.getElementById(\"varFN\").innerHTML ='$fn';</script>";
                        echo "<script>document.getElementById(\"varEM\").innerHTML ='$em';</script>";
                        echo "<script>document.getElementById(\"drpdw\").classList.remove(\"disnon\");</script>";
                        
                    }

                }
            }
                
            }
        }
    }
} 
?>


</body>


</html>
